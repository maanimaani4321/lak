...
namespace soundsystem {
    ...
    struct OboeInputStreamer : InputStreamer, public oboe::AudioStreamCallback
    {
    ...
    struct OboeOutputStreamer : OutputStreamer, public oboe::AudioStreamCallback
    {
        std::shared_ptr<oboe::AudioStream> stream;
        std::recursive_mutex mutex;
    ...
    private:
        OboeWrapper();
        OboeWrapper(const OboeWrapper& aud);
        const OboeWrapper& operator = (const OboeWrapper& aud);
    ...
    protected:
        ...
        // duplex (بدون تغییر جهت حفظ تطابق با ساختار پایه)
        duplexstreamer_t NewStream(StreamDuplex* duplex, int inputdeviceid,
                                   ...
                                   int output_channels, int framesize) override { return duplexstreamer_t(); }
        ...
        bool AddDuplexOutputStream(StreamDuplex* duplex,
                                   StreamPlayer* player) override { return false; }
