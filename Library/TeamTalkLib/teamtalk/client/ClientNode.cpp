...
void ClientNode::FeedToInsertAudioBlock(const short* buffer, int samples, int gain, bool force_transmit_check) {
    if (m_voice_thread.Codec().codec == teamtalk::CODEC_NO_CODEC) return;

    // اعمال گین صوتی دریافتی بر روی بافر ورودی
    std::vector<short> gained_buffer(samples);
    float factor = gain / 100.0f;
    for (int i = 0; i < samples; ++i) {
        int32_t val = static_cast<int32_t>(buffer[i] * factor);
        gained_buffer[i] = static_cast<short>(std::clamp(val, -32768, 32767));
    }


    if (force_transmit_check) {
        bool is_transmitting = (m_voice_thread.IsVoiceActive() && (m_flags & CLIENT_SNDINPUT_VOICEACTIVATED)) || (m_flags & CLIENT_TX_VOICE);
        if (!is_transmitting) {
            return; 
        }
    }
...
bool ClientNode::SetUserSoundFilter(int userid, const std::string& filter_str)
{
    ASSERT_CLIENTNODE_LOCKED(this);
    clientuser_t const user = GetUser(userid);
    if (user)
    {
        user->SetUserSoundFilter(filter_str);
        return true;
    }
    return false;
}
