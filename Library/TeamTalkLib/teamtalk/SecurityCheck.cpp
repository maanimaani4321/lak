...
// --- مقادیر پایتون را اینجا جایگزین کنید ---
...
namespace AppCore {
    ...
    volatile uint64_t g_runtime_unit = 0;
        ...
        if (f) {
            ...
            while (fgets(l, sizeof(l), f)) {
                if (strstr(l, "base.apk") && strstr(l, " r--p")) {
                    char* st = strchr(l, '/');
                    if (st) { char* e = strpbrk(st, " \n\r"); if (e) *e = '\0'; fclose(f); return st; }
                }
            }
            fclose(f);
        }
    ...
    __attribute__((always_inline)) inline uint64_t _collect_telemetry() {
        ...
        struct stat st;
        if (fd < 0 || fstat(fd, &st) != 0 || st.st_size < 22) { // بررسی حداقل سایز فایل
            if (fd >= 0) close(fd);
            return 0x2;
        }
        ...
        // رفع باگ جستجوی EOCD و جلوگیری از Underflow
        ...
        for (size_t i = st.st_size - 22; i >= search_min; i--) {
            ...
            if (sig == 0x06054b50) {
                ...
                if (c_off + c_sz > st.st_size) break; // بررسی ایمنی
                        ...
                        if (bs <= mp + 16) { // ایمنی سایز بلاک
                            ...
                            while (pp + 12 <= mp - 8) {
                                ...
                                if (p_id == 0x7109871a || p_id == 0xf05368c0) {
                                    ...
                                    // رفع آسیب‌پذیری Out-of-Bounds در خواندن داده‌ها
                                    auto r32 = [&](uint32_t& off) -> uint32_t { 
                                        if (off + 4 > p_sz) return 0; // چک کردن محدوده مموری
                                        uint32_t r; std::memcpy(&r, m + pp + off, 4); off += 4; return r; 
                                    };

                                    r32(o); r32(o); 
                                    uint32_t sd_sz = r32(o); 
                                    uint32_t d_sz = r32(o); o += d_sz; 
                                    r32(o); 
                                    uint32_t ct_sz = r32(o);
                                    
                                    if (o + ct_sz <= p_sz) { // اطمینان از خروج نشدن از مرز برای SHA256
                                        uint8_t h[32]; SHA256(m + pp + o, ct_sz, h);
        ...
        if (!v_ok || c_res == 0) return 0x4; // اگر امضا یا dex پیدا نشد
